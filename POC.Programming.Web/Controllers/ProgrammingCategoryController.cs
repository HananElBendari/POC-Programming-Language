...
namespace POC.Programming.Web.Controllers
{
    ...
    public class ProgrammingCategoryController : ControllerBase
    {
        ...
        /// <summary>
        /// Number of avaliable categories async
        /// </summary>
        /// <returns></returns>
        [HttpGet]
        [Route("[action]")]
        ...
        [ProducesResponseType(typeof(string), StatusCodes.Status500InternalServerError)]
        public async Task<ActionResult<List<ProgrammingCategoryDto>>> GetNumberOfAvaliableCategoriesAsync()
        {
            try
            {
                var result = await programmingCategoryService.GetNumberOfAvaliableCategoriesAsync();
                return Ok(result);
            }
            catch (Exception ex)
            {
